...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    private func makeDraft(image: UIImage, processed: ProcessedReceiptImage, recognized: RecognizedReceiptText, detection: ReceiptDetectionResult) { var d = ReceiptDraft(id: UUID(), sourceImage: image, processed: processed, imageRevision: UUID()); d.merchantName = detection.merchantCandidates.first?.value ?? ""; d.receiptDate = detection.dateCandidates.first?.value; d.currencyCode = detection.currencyCandidates.first?.value ?? preferences.homeCurrencyCode; d.subtotalText = detection.subtotalCandidates.first.map { "\($0.value)" } ?? ""; d.taxText = detection.taxCandidates.first.map { "\($0.value)" } ?? ""; d.totalText = detection.totalCandidates.first.map { "\($0.value)" } ?? ""; d.detectedCharges = detection.chargeCandidates.map { ReceiptChargeDraft(id: $0.id, label: $0.label, amount: $0.amount, percentage: $0.percentage, kind: $0.kind, confidence: $0.confidence, userClassification: defaultClassification(for: $0.kind)) }; d.recognizedText = recognized.fullText; d.warnings = detection.warnings; draft = d; stage = .confirmation }
    ...
    func saveReceipt() async -> UUID? {
        ...
        do {
            ...
            if case let .replaceImage(receiptID) = context {
                ...
                _ = try await repository.replaceImage(receiptID: receiptID, image: processed.fullImage); savedReceiptID = receiptID; savedDraftFingerprint = draft.fingerprint; stage = .completed; return receiptID
            }
