...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    let context: ReceiptScannerContext; private let recognizer: ReceiptTextRecognizing; private let parser: ReceiptFieldParsing; private let repository: ReceiptRepository; private let calculationRepository: CalculationRepository; private var processingTask: Task<Void, Never>?
    init(context: ReceiptScannerContext = .newReceipt, prefilledInput: TipCalculationInput? = nil, recognizer: ReceiptTextRecognizing = VisionReceiptTextRecognizer(), parser: ReceiptFieldParsing = ReceiptFieldParser(), repository: ReceiptRepository = FileReceiptRepository(), calculationRepository: CalculationRepository = FileCalculationRepository()) { self.context = context; self.recognizer = recognizer; self.parser = parser; self.repository = repository; self.calculationRepository = calculationRepository; if let input = prefilledInput { var d = ReceiptDraft(id: UUID()); d.currencyCode = input.currencyCode; d.subtotalText = input.subtotal.map(String.init(describing:)) ?? ""; d.taxText = input.tax.map(String.init(describing:)) ?? ""; d.totalText = input.finalTotal.map(String.init(describing:)) ?? ""; draft = d; stage = .confirmation } }
    ...
    func saveReceipt() async -> UUID? {
        ...
        if let savedReceiptID { return savedReceiptID }
        ...
        do {
            ...
            if case let .replaceImage(receiptID) = context {
                ...
                _ = try await repository.replaceImage(receiptID: receiptID, image: processed.fullImage); savedReceiptID = receiptID; stage = .completed; return receiptID
            }
            ...
            savedReceiptID = record.id; stage = .completed; return record.id
