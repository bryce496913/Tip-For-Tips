...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    func loadExistingReceiptIfNeeded() async {
        ...
        value.detectedCharges = receipt.detectedCharges.map { charge in .init(id: charge.id, label: charge.label, amountText: charge.amount.map { "\($0)" } ?? "", percentageText: charge.percentage.map { "\($0)" } ?? "", amount: charge.amount, percentage: charge.percentage, kind: charge.kind, confidence: charge.confidence, userClassification: charge.userClassification ?? .unreviewed, isIncludedInReceiptTotal: charge.isIncludedInReceiptTotal, source: charge.source) }
