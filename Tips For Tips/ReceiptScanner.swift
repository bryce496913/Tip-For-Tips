...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    func loadReplacementReceipt() async {
        guard case let .replaceImage(id) = context, draft == nil, let receipt = try? await repository.receipt(id: id) else { return }
        ...
        value.detectedCharges = receipt.detectedCharges.map { .init(id: $0.id, label: $0.label, amount: $0.amount, percentage: $0.percentage, kind: $0.kind, confidence: $0.confidence, userClassification: $0.userClassification ?? .unreviewed) }
    ...
    private func makeDraft(image: UIImage, processed: ProcessedReceiptImage, recognized: RecognizedReceiptText, detection: ReceiptDetectionResult) {
        ...
        if d.detectedCharges.isEmpty { d.detectedCharges = detection.chargeCandidates.map { ReceiptChargeDraft(id: $0.id, label: $0.label, amount: $0.amount, percentage: $0.percentage, kind: $0.kind, confidence: $0.confidence, userClassification: Self.initialClassification(for: $0.kind)) } }
        if d.recognizedText == nil { d.recognizedText = recognized.fullText }
        d.warnings = detection.warnings; draft = d; stage = .confirmation
    }
    var hasUnreviewedFinancialCharges: Bool { draft?.detectedCharges.contains { $0.userClassification == .unreviewed } == true }
...
struct ReceiptConfirmationView: View { @ObservedObject var model: ReceiptScannerViewModel; @State private var showImage = false
    ...
    @ViewBuilder private var actions: some View { ThemedCard { if model.isReplacementOnly { Text("Only the image and thumbnail will change. Saved merchant, amounts, charges, notes, recognized text, dates, and links remain unchanged.").appFont(.body); PrimaryButton(title: model.isSaving ? "Replacing…" : "Replace Receipt Image", systemImage: "photo.badge.arrow.down", isDisabled: model.isSaving || model.draft?.processed == nil || model.replacementCompleted) { Task { _ = await model.saveReceipt() } } } else { PrimaryButton(title: "Continue to Tip Assistant", systemImage: "sparkles") { model.continueToAssistant() }; PrimaryButton(title: model.isSaving ? "Saving…" : "Save Receipt", systemImage: "checkmark", isDisabled: model.isSaving || model.hasUnreviewedFinancialCharges) { Task { _ = await model.saveReceipt() } }; SecondaryButton(title: "Save and Calculate", systemImage: "arrow.right", isDisabled: model.isSaving || model.hasUnreviewedFinancialCharges) { model.saveAndCalculate() }; SecondaryButton(title: "Enter Values Manually", systemImage: "square.and.pencil") { model.message = "Edit any field above. OCR is optional." }; SecondaryButton(title: "Rescan", systemImage: "camera") { model.chooseCamera() } } } }
}
