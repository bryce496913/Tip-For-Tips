...
enum ReceiptScannerContext: Hashable { case newReceipt, attachToCalculation(UUID), replaceImage(UUID) }
...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    let context: ReceiptScannerContext; private let preferences: UserPreferences; private let recognizer: ReceiptTextRecognizing; private let parser: ReceiptFieldParsing; private let repository: ReceiptRepository; private let calculationRepository: CalculationRepository; private var processingTask: Task<Void, Never>?
    ...
    func saveReceipt() async -> UUID? {
        ...
        do {
            if let savedReceiptID, let existing = try await repository.receipt(id: savedReceiptID) {
                ...
                updated.imageFilename = existing.imageFilename; updated.thumbnailFilename = existing.thumbnailFilename
                updated.createdAt = existing.createdAt; updated.updatedAt = Date()
                try await repository.saveReceipt(updated); stage = .completed; return savedReceiptID
            }
