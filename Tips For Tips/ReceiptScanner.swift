...
@MainActor final class ReceiptScannerViewModel: ObservableObject {
    ...
    let context: ReceiptScannerContext; private let recognizer: ReceiptTextRecognizing; private let parser: ReceiptFieldParsing; private let repository: FileReceiptRepository; private var processingTask: Task<Void, Never>?
    init(context: ReceiptScannerContext = .newReceipt, recognizer: ReceiptTextRecognizing = VisionReceiptTextRecognizer(), parser: ReceiptFieldParsing = ReceiptFieldParser(), repository: FileReceiptRepository = FileReceiptRepository()) { self.context = context; self.recognizer = recognizer; self.parser = parser; self.repository = repository }
    ...
    func startManualEntry() { draft = ReceiptDraft(id: UUID(), sourceImage: nil, processed: nil, recognizedText: nil, warnings: []); stage = .confirmation }
    ...
    func saveReceipt() async -> UUID? { guard !isSaving, let draft else { return savedReceiptID }; isSaving = true; stage = .saving; defer { isSaving = false }; do { let record = record(from: draft, id: savedReceiptID ?? draft.id); if let processed = draft.processed { _ = try await repository.create(draft: record, fullImage: processed.fullImage, thumbnail: processed.thumbnail) } else { try await repository.createMetadataOnly(draft: record) }; savedReceiptID = record.id; stage = .completed; return record.id } catch { message = ReceiptScannerError.saveFailed.localizedDescription; stage = .confirmation; return nil } }
...
struct ReceiptConfirmationView: View { @ObservedObject var model: ReceiptScannerViewModel; @State private var showImage = false
    ...
    private func charges(_ draft: Binding<ReceiptDraft>) -> some View { ThemedCard { Text("Charges and gratuity").appFont(.title2); if draft.detectedCharges.isEmpty { Text("No gratuity or service-charge wording was detected. You can still calculate manually.").appFont(.body) }; ForEach(draft.detectedCharges) { $charge in VStack(alignment: .leading, spacing: AppSpacing.small) { Text(charge.label).appFont(.headline); Text(charge.kind.explanation).appFont(.body); Picker("Classification", selection: $charge.userClassification) { ForEach(ReceiptChargeClassification.allCases) { Text($0.title).tag($0) } }.pickerStyle(.menu).accessibilityLabel("Classification for \(charge.label)"); if DetectionConfidence(charge.confidence) != .high { Label(DetectionConfidence(charge.confidence).reviewText, systemImage: "exclamationmark.triangle").appFont(.body).foregroundStyle(AppTheme.highlight) } }.padding(.vertical, 6) }; TextField("Notes", text: draft.notes, axis: .vertical).textFieldStyle(AppTextFieldStyle()) } }
