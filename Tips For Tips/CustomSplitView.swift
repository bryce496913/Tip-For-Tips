...
struct CustomSplitView: View {
    ...
    var body: some View {
        NavigationStack {
            AppScreen { ScrollView { VStack(spacing: 16) { ScreenTitle(text: "Custom Split"); ForEach($people) { $person in PersonRow(person: $person, onDeletePerson: { personPendingDelete = person }) }; PrimaryButton(title: "Add Person", systemImage: "plus") { addPerson() }; ThemedCard { Text("Bill Total: \(totalAmount as NSDecimalNumber, formatter: Self.currencyFormatter)").appFont(.title2).foregroundStyle(AppTheme.highlight) } }.padding(20) } }
