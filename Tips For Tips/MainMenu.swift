            ...
            AppScreen {
                ScrollView {
                    VStack(spacing: AppSpacing.large) {
                        ...
                        ThemedCard {
                            Text("Additional tools").appFont(.h2)
...
struct DifficultyToggle: View { let title: String; let flag: FoodDeliveryDifficulty; @Binding var selection: FoodDeliveryDifficulty; var body: some View { Toggle(title, isOn: Binding(get: { selection.contains(flag) }, set: { isSelected in if isSelected { selection.insert(flag) } else { selection.remove(flag) } })).appFont(.paragraph) } }
...
struct SettingsView: View { let preferences: UserPreferences; var body: some View { AppScreen { ScrollView { VStack(spacing: AppSpacing.section) { ScreenTitle(text: "Settings", subtitle: "Review app defaults, privacy, and local data controls."); ThemedCard { Text("Defaults").appFont(.h2); ResultSummaryRow(label: "Home currency", value: preferences.homeCurrencyCode); ResultSummaryRow(label: "Default tip", value: "\(preferences.defaultTipPercentage)%"); ResultSummaryRow(label: "Tip basis", value: preferences.tipCalculationBasis.title); ResultSummaryRow(label: "Default people", value: "\(preferences.defaultPeopleCount)"); ResultSummaryRow(label: "Show explanations", value: preferences.showTippingExplanations ? "On" : "Off") }; ThemedCard { Text("Privacy and local data").appFont(.h2); Text("Saved calculations, receipts, notes, and preferences are stored locally on this device. Receipt text recognition uses on-device Apple Vision when scanning is available.").appFont(.paragraph); NavigationLink("Export data", value: AppRoute.history); NavigationLink("Delete saved activity", value: AppRoute.history) }; ThemedCard { Text("About").appFont(.h2); ResultSummaryRow(label: "App", value: "Tips for Tips"); ResultSummaryRow(label: "Version", value: Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1") } }.padding(AppSpacing.screen) } }.navigationTitle("Settings") } }
