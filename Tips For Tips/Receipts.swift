...
struct SavedReceiptsView: View {
    ...
    var body: some View {
        AppScreen { ScrollView { if viewModel.records.isEmpty { EmptyStateView(systemImage: "receipt", title: "No saved receipts", message: "Saved receipt photos will appear here.") } else { LazyVGrid(columns: columns, spacing: AppSpacing.section) { ForEach(viewModel.records) { receipt in NavigationLink(destination: FullImageView(viewModel: viewModel, record: receipt)) { ThemedCard { ReceiptImageView(viewModel: viewModel, record: receipt, thumbnail: true).frame(maxHeight: 140).clipShape(RoundedRectangle(cornerRadius: 12)); Text(receipt.displayName).appFont(.paragraph).foregroundStyle(AppTheme.text).lineLimit(2); HStack { Button("Rename") { renameText = receipt.displayName; renameRecord = receipt }.accessibilityLabel("Rename receipt \(receipt.displayName)"); Button("Delete", role: .destructive) { deleteRecord = receipt }.accessibilityLabel("Delete receipt \(receipt.displayName)") } } }.accessibilityLabel("Open receipt named \(receipt.displayName)") } }.padding(AppSpacing.screen) } } }
